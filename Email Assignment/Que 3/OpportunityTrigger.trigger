trigger OpportunityTrigger on Opportunity (before update, after update) {
    ...
    if (Trigger.isAfter && Trigger.isUpdate) {
        ...
        Map<Id, User> ownerMailMap = new Map<Id, User>([SELECT Id, Email FROM User WHERE Id IN :ownerIds]);
                
        List<Messaging.SingleEmailMessage> mailList = new List<Messaging.SingleEmailMessage>();
        EmailTemplate templateId = [SELECT Id FROM EmailTemplate WHERE Name = 'Opportunity Status Changed'];
        for (Opportunity opportunity : Trigger.new) {
            if (opportunity.Custom_Status__c != null && opportunity.Custom_Status__c != Trigger.OldMap.get(opportunity.Id).Custom_Status__c) {
                Messaging.SingleEmailMessage mail = new Messaging.SingleEmailMessage(); 
                String[] toAddresses = new String[] {ownerMailMap.get(opportunity.OwnerId).Email};
